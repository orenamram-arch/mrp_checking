...
(גרסה מתוקנת: תיקון באג תאריכים בתוכנית ההרכבה, תיקון באג NaN בהמרות מספריות,
ותיקון שליפת ETA בסיסי מה-MRP. ראו הערות "תיקון קריטי" לאורך הקובץ)
"""
...
אבל NaN הוא "truthy" בפייתון (bool(float('nan')) == True), ולכן התבנית הישנה
'pd.to_numeric(x, errors='coerce') or 0' לא באמת הופכת NaN ל-0 — היא רק
תופסת ערכים "falsy" כמו 0/None/''. זה גרם לכך שערכי NaN "דלפו" קדימה
בחישובי החוסרים ובחישובי המלאי, ולפעמים הסתירו חוסרים אמיתיים (כי NaN < 0
הוא תמיד False). safe_num מטפל בזה נכון עם pd.isna().
"""
