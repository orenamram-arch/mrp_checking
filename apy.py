...
st.markdown("דשבורד ויזואלי מתקדם לניהול חוסרים, ניתוח תוכניות ייצור וסימולציית Clear To Build מבוססת ETA מדויק מהאקסל")
...
# שורת התאריכים באקסל הגולמי (שורה 30 באקסל = אינדקס 2 או שורה 2 בהתאם למבנה, כאן שורה 2 באינדקסים)
raw_eta_dates = df_raw.iloc[2, 80:132].values
...
def get_first_supply_eta(pn):
    ...
    # נחפש את שורת הפריט בטבלה הגולמית ונרכוש את הערך הראשון הגדול מ-0 מעמודות ה-ETA (החל מעמודה 80 ועד 132)
    matching_rows = df_raw[df_raw.iloc[:, 1].astype(str).str.strip() == str(pn).strip()]
    if not matching_rows.empty:
        row_idx = matching_rows.index[0]
        for c_idx, col_pos in enumerate(range(80, 132)):
            try:
                val = df_raw.iloc[row_idx, col_pos]
                if pd.notnull(val) and val != '' and val != 'NaN':
                    q = float(val)
                    if q > 0:
                        # מציאת התאריך משורה 30 (שורה 2 בקובץ)
                        if c_idx < len(raw_eta_dates):
...
with tab2:
    ...
    st.markdown("המערכת מחלצת את מועד ההגעה הראשון ישירות מעמודות ה-ETA באקסל לפי שורת התאריכים. הפריט הקריטי מודגש ב-**BOLD**.")
