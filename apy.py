...
גרסה מלאה ומושלמת המעודכנת לפי סדר ההרכבות המדויק מהתמונה,
תחילת ייצור מספטמבר ואילך, וניהול מלאי וסימולציות בענן.
...
# סדר ההרכבות המדויק כפי שסופק בתמונה
...
if "custom_assembly_plan_df" not in st.session_state:
    ...
    for r in range(3, df_raw.shape[0]):
        ...
        if pd.notnull(asm_pn):
            ...
            # בדיקה האם ההרכבה נמצאת ברשימה המדויקת שסופקה
...
# הגדרת רשימת ההרכבות החוקיות לפי הסדר המדויק מהתמונה
...
for asm in valid_assemblies:
    # מציאת רמת העץ מתוך עמודת ההרכבה בקובץ או מיפוי ברירת מחדל
    try:
        col_idx = df.columns.get_loc(asm)
        lvl_val = int(df_levels.iloc[0, col_idx])
        assembly_levels[asm] = lvl_val
    except:
...
level_options = ["הכל"] + sorted(list(set([str(assembly_levels.get(c, 0)) for c in valid_assemblies])))
...
with tab2:
    st.markdown(f'<div class="section-title">📊 סימולציית Clear To Build (CTB) לפי הסדר המדויק מהתמונה</div>', unsafe_allow_html=True)
    st.markdown("טבלה זו מציגה את ההרכבות לפי סדר הרמות והמקטים כפי שהוגדר בדיוק.")
    ...
    matrix_rows = []
    chart_assembly_data = []
    # שימוש ברשימה המדויקת לפי הסדר
    assemblies_to_check = [asm for asm in EXACT_ASSEMBLIES_ORDER if selected_assembly == "הכל" or asm == selected_assembly]
    ...
    for asm_col in assemblies_to_check:
        ...
        if has_any_build or asm_col in filtered_assembly_cols:
    ...
    if matrix_rows:
        ...
        if chart_assembly_data:
            ...
            st.markdown("##### 📈 השוואת תוכנית ייצור לפי הסדר המדויק")
            fig_bar_asm = px.bar(
                chart_melted,
                x="הרכבה ותיאור",
                y="כמות",
        ...
        st.info("לא נמצאו הרכבות בטווח החודשים שנבחר.")
...
with tab5:
    st.markdown(f'<div class="section-title">🏭 ניהול WIP חכם (לפי סדר ההרכבות המדויק)</div>', unsafe_allow_html=True)
    st.markdown("כאן ניתן להוריד הרכבות לייצור לפי רשימת ההרכבות והסדר המדויק.")

    wip_current = fetch_wip_records()

    if wip_current:
        st.markdown("##### 🏁 סגירת הרכבות שהיו ב-WIP")
        with st.form("close_wip_form"):
            ...
            if st.form_submit_button("סגור WIP והוסף למלאי הזמין"):
                if is_finished:
                    ...
                    st.success(f"ההרכבה `{wip_to_close}` טופלה בהצלחה!")
                    st.rerun()
                else:
    ...
    with st.form("wip_form"):
        ...
        if current_wip_val > 0:
            st.info(f"ℹ️ שים לב: הרכבה זו כבר קיימת ב-WIP בכמות של `{current_wip_val}`.")
        ...
        if submitted_wip:
            ...
            st.success(f"🚀 ההרכבה נוספה בהצלחה ל-WIP!")
            st.rerun()
...
with tab6:
    ...
    if selected_pn != "הכל":
        ...
        st.info(f"ℹ️ מועד ה-ETA המקורי והכמות שעלו מדוח ה-MRP עבור מק'ט זה הם: **{base_mrp_eta}** | כמות: **{base_mrp_qty:g}**")
...
with tab7:
    ...
    if not eta_df.empty:
        st.dataframe(eta_df, use_container_width=True, height=450)
            ...
            with st.form("edit_mgmt_form"):
                ...
                with col_e1:
                    if st.form_submit_button("💾 שמור שינוי כמות"):
                        ...
                        st.success(f"הכמות למק'ט `{selected_mgmt_pn}` עודכנה בהצלחה!")
                        st.rerun()
                with col_e2:
...
with tab10:
    st.markdown('<div class="section-title">🎯 ניתוח רגישות וניהול תוכנית הייצור לפי סדר ההרכבות</div>', unsafe_allow_html=True)
    st.markdown("כאן מוצגת תוכנית העבודה (המתחילה מספטמבר ואילך) בהתאם לסדר ההרכבות המדויק מהתמונה.")
    ...
    if not assembly_plan_df.empty:
        ...
        # סידור ההרכבות בדיוק לפי סדר EXACT_ASSEMBLIES_ORDER
        pivot_plan_df["Assembly_PN"] = pd.Categorical(pivot_plan_df["Assembly_PN"], categories=EXACT_ASSEMBLIES_ORDER, ordered=True)
        pivot_plan_df = pivot_plan_df.sort_values("Assembly_PN").reset_index(drop=True)
        ...
        with st.form("update_plan_form"):
            ...
            if st.form_submit_button("כן, עדכן ושמור את התוכנית החדשה"):
                if update_confirmation:
                    ...
                    st.success("✅ תוכנית העבודה עודכנה בהצלחה!")
                    st.rerun()
                else:
